...
public class CryptoManager {
     ...
     * an offset
     * 
     * and each character in plainText is replaced by the character \"offset\" away
     * from it
     * 
	...
	for (int i = 0; i < temp.length(); i++) {
	    ...
	    replacer = temp.charAt(i);
	    replacer += key;
	    replacer = (char) ((replacer - LOWER_RANGE) % RANGE + LOWER_RANGE);

	    temp.setCharAt(i, replacer);
     ...
     * is offset
     * 
     * according to the ASCII value of the corresponding character in bellasoStr,
     * which is repeated
     * 
	...
	for (int i = 0; i < temp.length(); i++) {
	    ...
	    replacer = temp.charAt(i);
	    replacer += bellasoStr.charAt(i % bellasoStr.length());
	    replacer = (char) ((replacer - LOWER_RANGE) % RANGE + LOWER_RANGE);
     ...
     * an offset
     * 
     * and each character in encryptedText is replaced by the character \"offset\"
     ...
     * encryptedText is replaced by
     * 
     * the character corresponding to the character in bellasoStr, which is repeated
     * 
     * to correspond to the length of plainText. This is the inverse of the
     * encryptBellaso method.
     * 
	...
	var temp = new StringBuffer();
	    ...
	    for (int i = 0; i < temp.length(); i++) {
	        replacer = temp.charAt(i);
	        replacer -= bellasoStr.charAt(i % bellasoStr.length());
	        replacer = (char) ((replacer - LOWER_RANGE + RANGE) % RANGE + LOWER_RANGE);
